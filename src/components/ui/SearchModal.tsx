...
export default function SearchModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  useGSAP(() => {
    if (isOpen) {
      ...
      gsap.fromTo('.search-backdrop', { opacity: 0 }, { opacity: 1, duration: 0.3 });
      gsap.fromTo('.search-dialog', { opacity: 0, scale: 0.95, y: -20 }, { opacity: 1, scale: 1, y: 0, duration: 0.4, ease: "back.out(1.5)" });
    } else if (shouldRender) {
      const tl = gsap.timeline({ onComplete: () => setShouldRender(false) });
      tl.to('.search-dialog', { opacity: 0, scale: 0.95, y: -10, duration: 0.3, ease: "power2.in" }, 0);
      tl.to('.search-backdrop', { opacity: 0, duration: 0.3 }, 0);
    }
  }, [isOpen]);
  ...
  return (
    <div ref={containerRef}>
      <div
        onClick={onClose}
        className="search-backdrop fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm"
      />
      <div className="fixed inset-0 z-[101] flex items-start justify-center pt-[15vh] px-4 pointer-events-none">
        <div
          className="search-dialog w-full max-w-2xl bg-white/80 backdrop-blur-3xl rounded-3xl shadow-[0_30px_60px_-15px_rgba(0,100,148,0.3)] overflow-hidden border border-white/60 pointer-events-auto flex flex-col"
